...
// identifier는 record_id에 WP_를 붙여 사용
        ...
        $identifier = $_POST["date_from"] . "_" . $_POST['title'] . "_" . $record_id;
        ...
        // 파일이 이동할 경로
        $upload_file_path = $upload_file_dir . $identifier;
...
//    print "등록이 완료되었습니다." . $upload_file_path;
        ...
        $sql = "INSERT INTO rikarsong.record (identifier, record_id, title, creator, collector, date_from, date_to, extent, medium, scope, type, description, level_of_description, url, delete_Yn)
            VALUES (:identifier, :record_id, :title, :creator, :collector, :date_from, :date_to, :extent, :medium, :scope, :type, :description, 'item', :url, 'N')";
        $stmh = $pdo->prepare($sql);
...
// identifier는 record_id에 WP_를 붙여 사용
            ...
            $identifier = $_POST["date_from"] . "_" . $title . "_" . $record_id;
            // 파일 업로드
            // 업로드 될 파일이 이동할 디렉토리의 경우 서버 환경에 맞게 재설정 필요
            // 원격 웹 호스팅 환경
            $upload_file_dir = '../upload/';
